...
interface CreateAttributeModalProps {
  ...
  onCreateAttribute: (attribute: Omit<Attribute, "id">) => void;
  isLoading: boolean;
}
...
export function CreateAttributeModal({
  ...
  const handleAddColor = () => {
    ...
    const nameExists = colorValues.some(
      (c) => c.name.toLowerCase() === currentColorName.toLowerCase(),
    );
    const colorExists = colorValues.some((c) => c.value === currentColor);
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6 py-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="text-sm text-muted-foreground">
                      {currentColor || "Ningún color seleccionado"}
                    </span>
                  </div>
                  {isColorDuplicate && (
                    <p className="text-xs text-destructive">
          ...
          {attributeType === "Genero" && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <Label>
                  Seleccionar opciones de género ({selectedGenders.length}/10)
                </Label>
                {selectedGenders.length >= 10 && (
                  <span className="text-xs text-amber-600">
                    Máximo alcanzado
                  </span>
                )}
        ...
        <div className="flex justify-end gap-3 pt-4 border-t">
          ...
          <Button onClick={handleSubmit} disabled={isLoading}>
            {isLoading ? "Creando..." : "Crear Atributo"}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
